<?php
  ...
  class Suppliers{
    ...
    public function get_supdata($search_by,$value)
    {
    ...
    public function getSuptData($search_by,$value)
    {
    ...
    public function save($id,$name,$address,$tel,$mobile,$notes)
  	{
        $message ='';
  	    $this->con_state = $this->Check_Connection();
  		  if ($this->con_state) {
  			$db = new Database();
  		    if($id == NULL){ // Insert
              $fields = "(supName,Address,Telephone,mobile,Notes)";
  		      $values = "( '".$name."' ,  '".$address."' , '".$tel."' , '".$mobile."' , '".$notes."' )";
    ...
    public function update_sup($fileds,$values,$counter,$select,$id)
    {
        $message ='';
        $this->con_state = $this->Check_Connection();
        if ($this->con_state) 
